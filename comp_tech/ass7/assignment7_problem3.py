...
def efficient_linear_scan(X, Q, b=None):
    """
    Perform linear scan for querying nearest neighbor with batch processing.
    This method uses matrix multiplication to compute distances more efficiently.
    X: n*d dataset
    Q: m*d queries
    ...
    for batch in range(num_batches):
        ...
        Q_batch_squared_norms = np.sum(Q_batch**2, axis=1)

        # Compute the dot product betwee n X and Q_batch.T
        dot_product = np.dot(X, Q_batch.T)

        # Compute the distance matrix using the formula
        dists = X_squared_norms[:, np.newaxis] + Q_batch_squared_norms[np.newaxis, :] - 2 * dot_product

        # Find the indices of the minimum distances
        I[start:end] = np.argmin(dists, axis=0)
...
if __name__ == '__main__':
    ...
    I = efficient_linear_scan(X, Q, args.batch_size)
    t7 = time.time()
    assert I.shape == (m,)
